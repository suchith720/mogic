...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/mogic/16_ngame-hab-linker-for-wikiseealsotitles-001'
    
    pkl_file = f'{pkl_dir}/processed/wikiseealsotitles_data-hab_distilbert-base-uncased_sxc.pkl'
    if build_block:
        config = load_config('configs/wikiseealsotitles-hyperlink-abstract.json')
        block = SXCBlock.from_cfg(config, tokenizer="distilbert-base-uncased", padding=True,
                                  return_tensors='pt', main_max_data_sequence_length=32, n_slbl_samples=1, main_oversample=False,
    ...
    linker_block = block.linker_dset('hab_meta')
    ...
    if do_prediction:
        ...
        pred = get_output(o.pred_idx, o.pred_ptr, o.pred_score, n_lbl=block.test.dset.n_lbl)
        sp.save_npz(f'{pred_dir}/test_predictions.npz', pred)

        # dset = block.test.dset.data
        # inference_dset = block.inference_dset(dset.data_info, dset.data_lbl, dset.lbl_info, dset.data_lbl_filterer)

        # learn = XCLearner(
        #     model=model, 
        #     args=args,
        #     eval_dataset=inference_dset,
        #     data_collator=linker_block.collator,
        #     compute_metrics=metric,
        # )

        # o = learn.predict(inference_dset)

        # pred_dir = f'{output_dir}/predictions'
        # os.makedirs(pred_dir, exist_ok=True)

        # with open(f'{pred_dir}/linker_predictions.pkl', 'wb') as file:
        #     pickle.dump(o, file)

        # pred = get_output(o.pred_idx, o.pred_ptr, o.pred_score, n_lbl=block.n_lbl)
        # sp.save_npz(f'{pred_dir}/linker_predictions.npz', pred)
