...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/mogic/17_entity-ngame-linker-for-msmarco-with-query-label-input'
    config_file = 'configs/msmarco-entity.json'

    """ Load data """
    pkl_file = f'{pkl_dir}/processed/msmarco_data-entity1_distilbert-base-uncased_sxc-qdoc.pkl'
    ...
    if build_block:
        ...
        label_text = block.train.dset.data.lbl_info['input_text']

        trn_file = '/home/aiscuser/scratch1/outputs/mogic/17_ngame-for-msmarco-001/predictions/train_predictions.npz'
        trn_text = get_query_lbl_text(block.train.dset.data.data_info['input_text'], label_text, trn_file, topk=3) 

        val_file = '/home/aiscuser/scratch1/outputs/mogic/17_ngame-for-msmarco-001/predictions/test_predictions.npz'
        val_text = get_query_lbl_text(block.test.dset.data.data_info['input_text'], label_text, val_file, topk=3)

        block.train.dset.data.data_info['input_text'] = trn_text
        block.test.dset.data.data_info['input_text'] = val_text

        from transformers import AutoTokenizer
        tokz = AutoTokenizer.from_pretrained("distilbert-base-uncased")

        block.train.dset.data.data_info.update(tokz(trn_text, padding=True, return_tensors='pt', truncation=True, max_length=256))
        block.test.dset.data.data_info.update(tokz(val_text, padding=True, return_tensors='pt', truncation=True, max_length=256))
