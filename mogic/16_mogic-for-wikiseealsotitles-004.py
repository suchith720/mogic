...
from xcai.models.oak import OAK014
...
def get_metadata_remap(embeddings:torch.Tensor, cluster_sz:int=3):
    clusters = BalancedClusters.proc(embeddings.half(), min_cluster_sz=cluster_sz)

    remapped_embeddings = []
    remap = torch.zeros(embeddings.shape[0], dtype=torch.int64)
    for i,o in enumerate(clusters): 
        remap[o] = i
        remapped_embeddings.append(embeddings[o].mean(dim=0, keepdim=True))

    return torch.vstack(remapped_embeddings), remap, len(clusters)
...
if __name__ == '__main__':
    ...
    output_dir = '/home/aiscuser/scratch1/outputs/medic/31_mogic-for-wikiseealsotitles-004'
    ...
    meta_embeddings, meta_remap, n_meta_clusters = get_metadata_remap(meta_embeddings, cluster_sz=2)

    """ Student model """
    bsz = max(args.per_device_train_batch_size, args.per_device_eval_batch_size)*torch.cuda.device_count()

    m_student = OAK014.from_pretrained('sentence-transformers/msmarco-distilbert-base-v4', batch_size=bsz, num_batch_labels=5000,
                               margin=0.3, num_negatives=10, tau=0.1, apply_softmax=True,

                               data_aug_meta_prefix='lnk2data', lbl2data_aug_meta_prefix=None,
                               data_pred_meta_prefix=None, lbl2data_pred_meta_prefix=None,

                               n_metadata=block.train.dset.meta['lnk_meta'].n_meta, resize_length=5000, n_meta_clusters=n_meta_clusters,
    ...
    m_student.encoder.set_metadata_remap(meta_remap)
